...
export default function procedureBuilder (stack: any) {
	...
	stack.forEach((el: CProcedure) => {
		...
		let requiredAirport = airportsProcedures.find((ap) => ap.AIRPORT_CODE === airport)
		if (!requiredAirport) {
			airportsProcedures.push({AIRPORT_CODE: airport, RUNWAYS: [], coordinates: [airportData.latitude, airportData.longitude], 'SID': [], 'STAR': [], 'APPROACH': []});
			requiredAirport = airportsProcedures.find(
				(ap) => ap.AIRPORT_CODE === airport
			);
		};
		switch(type) {
			case 'D':
				/*проверяем наличие схемы для конкретного аэропорта */
				const SIDitem = requiredAirport.SID.find((el) => el.name === procedureIdent);
				...
				if (!SIDitem) {
					...
					requiredAirport.SID.push({
						name: procedureIdent,
						runway: transitionIdent,
						path: [point],
